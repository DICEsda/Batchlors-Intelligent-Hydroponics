...
# Feature columns for growth prediction
FEATURES = [
    "days_since_planting",
    "air_temp_c",
    "humidity_pct",
...
def prepare_training_data(
    ...
    """
    logger.info(f"Fetching training data (last {hours} hours)")
    
    # Get height measurements (ground truth)
    ...
    if not tower_df.empty:
        # Aggregate telemetry by day for each tower
        tower_df["date"] = pd.to_datetime(tower_df["timestamp"]).dt.date
        tower_daily = tower_df.groupby(["tower_id", "date"]).agg({
            "air_temp_c": "mean",
            "humidity_pct": "mean",
            "light_lux": "mean",
        }).reset_index()
    ...
    if not reservoir_df.empty:
        # Add pH and EC from reservoir
        reservoir_df["date"] = pd.to_datetime(reservoir_df["timestamp"]).dt.date
        reservoir_daily = reservoir_df.groupby("date").agg({
            "ph": "mean",
            "ec_ms_cm": "mean",
        }).reset_index()
...
def train_growth_model(
    ...
    logger.success(f"Model trained - RMSE: {metrics['rmse']:.3f}, R²: {metrics['r2']:.3f}")
