...
class MongoDBConnector:
    ...
    def get_height_measurements(
        ...
        if hours or start_time or end_time:
            query = self._build_time_query(hours or 8760, start_time, end_time)  # Default 1 year
    ...
    def get_ml_dataset(
        ...
        if not reservoir_df.empty:
            ...
            numeric_cols = reservoir_df.select_dtypes(include=["float64", "int64"]).columns
            reservoir_agg = reservoir_df[numeric_cols].resample(resample_interval).mean()
            reservoir_agg.columns = [f"reservoir_{col}" for col in reservoir_agg.columns]
        else:
        ...
        logger.info(f"Created ML dataset with {len(combined)} samples and {len(combined.columns)} features")
        return combined
    ...
    def get_collection_stats(self) -> dict:
        ...
        for collection_name in ["tower_telemetry", "reservoir_telemetry", "height_measurements"]:
